...
engine_file_path = "/root/project/int8test/LP_RECOGNIZE_INT8/models_save/lp_int8.trt"
crnn_ocr =ocr_trt(engine_file_path)
crnn_ocr.destroy()
...
def lp_ocr(img):
    ...
    converter = strLabelConverter(alphabet = """京沪津渝冀晋蒙辽吉黑苏浙皖闽赣鲁豫鄂湘粤桂琼川贵云藏陕甘青宁新0123456789ABCDEFGHJKLMNPQRSTUVWXYZ港学使警澳挂军北南广沈兰成济海民航空""")
    ...
    # loc, conf, landms = retinaface.infer(img)
    # print(retinaface.infer(img)[0].shape)
    preds = crnn_ocr.infer(input)
    ...
    sim_pred,scores = converter.numpy_decode((preds_.data),preds,len_dict=None)
